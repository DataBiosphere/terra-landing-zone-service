...
class CromwellBaseResourcesDefinitionV1 extends LandingZoneDefinition {
  ...
  private final ClientLogger logger = new ClientLogger(CromwellBaseResourcesDefinitionV1.class);

  private List<InputParameterValidator> validators;

  protected CromwellBaseResourcesDefinitionV1(ArmManagers armManagers) {
    super(armManagers);
    validators = List.of(new AksParametersValidator(), new BlobCorsParametersValidator());
  }
  ...
  public LandingZoneDeployment.DefinitionStages.Deployable definition(
    ...
    validators.forEach(v -> v.validate(parametersResolver));
    ...
    var postgres =
        armManagers
            .postgreSqlManager()
            .servers()
            .define(nameGenerator.nextName(ResourceNameGenerator.MAX_POSTGRESQL_SERVER_NAME_LENGTH))
            .withRegion(resourceGroup.region())
            .withExistingResourceGroup(resourceGroup.name())
            .withProperties(
    ...
    var prerequisites =
        deployment
            ...
            .withVNetWithPurpose(
                vNet,
                CromwellBaseResourcesFactory.Subnet.AKS_SUBNET.name(),
                SubnetResourcePurpose.AKS_NODE_POOL_SUBNET)
            .withVNetWithPurpose(
                vNet,
                CromwellBaseResourcesFactory.Subnet.BATCH_SUBNET.name(),
                SubnetResourcePurpose.WORKSPACE_BATCH_SUBNET)
            .withVNetWithPurpose(
                vNet,
                CromwellBaseResourcesFactory.Subnet.POSTGRESQL_SUBNET.name(),
                SubnetResourcePurpose.POSTGRESQL_SUBNET)
            .withVNetWithPurpose(
                vNet,
                CromwellBaseResourcesFactory.Subnet.COMPUTE_SUBNET.name(),
    ...
    var privateEndpoint =
        azureResourceManager
            .privateEndpoints()
            .define(nameGenerator.nextName(ResourceNameGenerator.MAX_PRIVATE_ENDPOINT_NAME_LENGTH))
            .withRegion(resourceGroup.region())
            .withExistingResourceGroup(resourceGroup)
            .withSubnetId(
                vNetwork
    ...
    var aksPartial =
        azureResourceManager
            ...
            .withVirtualNetwork(
                vNetwork.id(), CromwellBaseResourcesFactory.Subnet.AKS_SUBNET.name());
