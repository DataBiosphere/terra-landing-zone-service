package bio.terra.landingzone.terraform.client;
...
final class ProcessLauncher {
  ...
  void appendCommands(String... commands) {
    Stream<String> filteredCommands =
        Arrays.stream(commands).filter(c -> c != null && c.length() > 0);
    ...
    if (value != null && value.length() > 0) {
      ...
      String target =
          (current == null || current.length() == 0
              ? value
              : String.join(delimiter, current, value));
    ...
    if (!this.inheritIO) {
      if (this.outputListener != null) {
        this.executor.submit(
            () -> this.readProcessStream(this.process.getInputStream(), this.outputListener));
      }
      if (this.errorListener != null) {
        this.executor.submit(
            () -> this.readProcessStream(this.process.getErrorStream(), this.errorListener));
      }
    }
